import React, { useState } from "react";
import { useSelector } from "react-redux";
import { Avatar } from "antd";

const Suggestions = () => {
  const allUsers = useSelector((state) => state.AllUserReducer.allUsers);
  const [isIncluded, setIncluded] = useState();
  const User = useSelector((state) => state.UserReducers.user);
  if (User.friends.length) {
    const mappedUserFriendsId = User.friends.map((id) => id._id);
    const NotToBeIncludedInSuggestionList = allUsers.filter(
      (user) => user._id.includes(mappedUserFriendsId) || user._id !== User._id
    );
    setIncluded(NotToBeIncludedInSuggestionList);
  }

  const handleAddFriend = async(id)=>{

  }

  return (
    <div className="mt-[10px]">
      <div>
        {isIncluded ? (
          <div></div>
        ) : (
          <div className="flex flex-col gap-y-[10px]">
            {allUsers
              ? allUsers
                  .filter((user) => user._id !== User._id)
                  .map((user) => (
                    <div key={user._id}>
                      <div className="flex justify-between">
                        <div className="flex gap-x-[7px] items-center">
                          <Avatar
                            src={
                              user.profilePic
                                ? user.profilePic
                                : "data:image/png;base64,"
                            }
                            size={50}
                          />
                          <p className="roboto-bold text-[20px] text-black">
                            {user.username}
                          </p>
                        </div>

                        <span onClick={()=> handleAddFriend(user._id)} className="bg-black w-fit p-[7px] rounded-[8px] h-[40px] cursor-pointer text-white text-[18px] roboto-medium">
                          Follow
                        </span>
                      </div>
                    </div>
                  ))
              : ""}
          </div>
        )}
      </div>
    </div>
  );
};

export default Suggestions;
